...
|정규식|문자열|매치여부|설명|
|---|---|---|---|
|[abc]|a|Y|"a"는 정규식과 일치하는 문자인 "a"가 있으므로 매치|
||before|Y|"before"는 정규식과 일치하는 문자인 "b"가 있으므로 매치|
||dude|N|"dude"는 정규식과 일치하는 문자인 a, b, c 중 어느 하나도 포함하지 않으므로 매치 X|

- [] 안의 두 문자 사이에 하이픈(-)을 사용하면 두 문자 사이의 범위(From-To)를 의미
- ex) [a-c]: [abc]와 동일한 의미, [0-5]: [012345]와 동일한 의미, [a-zA-Z]: 알파벳 모두
- 단, 문자 클래스 안에 ^ 메타 문자를 사용할 경우에는 반대(not)라는 의미
...
- 줄바꿈 문자를 재외한 모든 문자와 매치됨을 의미하는 메타 문자
- ex) a.b: a와 b 사이에 줄바꿈 문자를 제외한 어떤 문자가 들어가도 모두 매치
- "a + 모든 문자 + b"

|정규식|문자열|매치여부|설명|
|---|---|---|---|
|a.b|aab|Y|"aab"는 가운데 문자 a가 모든 문자를 의미하는 .과 일치하므로 정규식과 매치|
||a0b|Y|"a0b"는 가운데 문자 0이 모든 문자를 의미하는 .과 일치하므로 정규식과 매치|
||abc|N|"abc"는 a문자와 b문자 사이에 어떤 문자라도 하나는 있어야하는 이 정규식과 일치하지 않으므로 매치되지 않음

- 혼동하기 쉬운 예: 문자 클래스 [] 안에 Dot(.) 문자가 있는 경우
- a[.]b: a와 b 사이에 Dot(.) 문자가 있으면 매치
- 여기서 Dot(.) 문자는 '모든 문자'라는 의미가 아닌 문자 '.' 그대로를 의미
- "a + Dot(.)문자 + b"
- "a.b" 문자열은 매치, "a0b" 문자열과는 매치되지 않음
...
- m 또는 n은 생략 가능
- ex) {3,}: 반복 횟수가 3 이상인 경우, {,3}: 반복 횟수가 3 이하인 경우
- 생략된 m은 0과 동일, 생략된 n은 무한대(2억 개 미만)를 의미
